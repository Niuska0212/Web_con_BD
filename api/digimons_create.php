...
if (empty($nivel)) {
    http_response_code(400);
    echo json_encode(["error" => "El nivel es obligatorio."]);
    exit();
}
if (empty($atributo)) {
    http_response_code(400);
    echo json_encode(["error" => "El atributo es obligatorio."]);
    exit();
}
...
try {
    $stmt = $conn->prepare("INSERT INTO digimons (nombre, tipo, nivel, atributo, evoluciones) VALUES (:nombre, :tipo, :nivel, :atributo, :evoluciones)");
    ...
    echo json_encode(["message" => "Digimon agregado exitosamente.", "id" => $conn->lastInsertId()]);

} catch (PDOException $e) {
    http_response_code(500);
    echo json_encode(["error" => "Error al agregar Digimon: " . $e->getMessage()]);
}
?>
